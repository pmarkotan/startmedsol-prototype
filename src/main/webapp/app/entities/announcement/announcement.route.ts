...
export class AnnouncementResolve implements Resolve<IAnnouncement> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<IAnnouncement> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((announcement: HttpResponse<Announcement>) => {
...
export const announcementRoute: Routes = [
  {
    ...
    data: {
      ...
      defaultSort: 'id,asc',
      pageTitle: 'startMedsolPrototypeApp.announcement.home.title',
    },
